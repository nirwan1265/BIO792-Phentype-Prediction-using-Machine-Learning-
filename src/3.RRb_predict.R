RRb_func = function (train_pheno, train_geno, test_geno) {
  ...
  train_predicted = train_valid + c(train_BLUP$beta)
  val_valid = test_geno %*% train_e
  val_predicted <- val_valid + c(train_BLUP$beta)
  return_value = list("val_predicted"=val_predicted, "train_predicted"=train_predicted, "model"=train_BLUP, "u"=train_BLUP$u, "beta"=train_BLUP$beta)
  return(return_value)
}
